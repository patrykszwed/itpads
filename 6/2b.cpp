...
int main(int argc, char **argv) {
    ...
    if (processNumber == 0) {
        mainArray = getRandomValuesVector(chunkSize * maximumNumberOfProcesses + secondOffset);
    }
    ...
    for (int i = 0; i < numberOfElements / 2; ++i) {
        if (processNumber == 0) {
            int position = 0;
            for (int j = 0; j < 2; ++j) {
                subArray[j * 2] = mainArray[i * 2 + j + firstOffset];
                subArray[j * 2 + 1] = mainArray[i * 2 + j + secondOffset];
            }
        ...
        MPI_Gatherv(multiplicationArray, amountsReceived[processNumber], MPI_INT,
                    processNumber == 0 ? mainArray + i * 2 : NULL,
    ...
    if (processNumber == 0) {
        ...
        for (int i = 0; i < chunkSize * maximumNumberOfProcesses + secondOffset; ++i) {
            cout << mainArray[i] << ", ";
        }
        cout << endl;
