...
int main(int argc, char **argv) {
    ...
    if (process_number == first_process) {
        ...
        sendNumber(number, process_number, (process_number +1) % number_of_processes);
    }
...
void sendNumber(int& number_to_send, int source_process, int destination_process) {
    printf("Process with number %d will send a number to the process with number %d \n",
...
void receiveNumber(int& variable_to_save, int source_process, int destination_process) {
    printf("Process with number %d will receive a number from the process with number %d \n",
